...
namespace NP
{
	...
	namespace Global
	{
		...
		private:
			//
			typedef std::vector<std::pair<Job_index, std::vector<Job_index>>> blocking_list; // Need to change direction to blocked -> blocking because this access is too expensive
			typedef std::vector<std::vector<Job_index>> blocked_by_list; //Indices are idx
			//typedef std::vector<std::pair<Job_index, std::pair<Time, Time>>> saved_start_times;
			typedef std::vector<std::pair<Time, Time>> saved_start_times; //Indices are idx
			//
		...
		public:
			//
			blocking_list blocked_by;
			blocked_by_list blocked_by_reverse; //Correct way of accessing
			saved_start_times saved_starts;
			//
			...
			Schedule_state(const unsigned int num_processors, const State_space_data<Time> &state_space_data)
				: core_avail{num_processors, Interval<Time>(Time(0), Time(0))}, certain_jobs{}, earliest_certain_successor_job_disptach{Time_model::constants<Time>::infinity()}, earliest_certain_gang_source_job_disptach{state_space_data.get_earliest_certain_gang_source_job_release()}
			{
				assert(core_avail.size() > 0);
				blocked_by_reverse.resize(state_space_data.jobs.size());
				saved_starts.resize(state_space_data.jobs.size());
				for(auto rj: state_space_data.jobs)	
					DM("Job " << rj.get_job_index() << " is " << rj << std::endl);
			}
				...
				unsigned int ncores = 1)
			{
				...
				// save the job finish time of every jb with a successor that is ntot executed yet in the current state
				...
				DM("scheduled jobs:" << scheduled_jobs << std::endl);
				DM("edge:" << state_space_data.jobs[j] << std::endl);
				
				blocked_by_reverse = from.blocked_by_reverse; //ChatGPT says this is deep copy, need to make sure
				saved_starts = from.saved_starts;
				const Job edge = state_space_data.jobs[j];
				std::vector<Job_index> blocks;
				for (const auto rj : state_space_data.jobs)
				{
	
					Time blocking_a_min = edge.earliest_arrival();
					Time blocking_a_max = edge.latest_arrival();
					Time blocked_a_min = rj.earliest_arrival();
					Time blocked_a_max = rj.latest_arrival();
					Time blocking_deadline = edge.get_deadline();
					Time blocked_deadline = rj.get_deadline();

					//Can also use priority_exceeds here because we have tie breakers
					//if (blocking_deadline > blocked_a_min && blocked_deadline > blocking_a_min && !scheduled_jobs.contains(rj.get_job_index()) && rj.higher_priority_than(edge))
					if (blocking_deadline > blocked_a_min && blocked_deadline > blocking_a_min && !scheduled_jobs.contains(rj.get_job_index()) && rj.priority_exceeds(edge.get_priority()))
					{	
						std::cout << "edge priority: " << edge.get_priority() << " rj priority: " << rj.get_priority() << std::endl;
						std::cout << "rj higher priority than edge: " << rj.higher_priority_than(edge) << std::endl;
						std::cout << "Found blocking! -- Blocking id: " << edge.get_job_index() << " Blocked id: " << rj.get_job_index() << std::endl;
						blocked_by_reverse[rj.get_job_index()].push_back(edge.get_job_index());
					}

					if (blocking_deadline > blocked_a_min && blocked_deadline > blocking_a_min && !scheduled_jobs.contains(rj.get_job_index()))
					{
						blocks.push_back(rj.get_job_index());
					}
				}
				blocked_by.push_back(std::make_pair(j, blocks));
				//saved_starts.push_back(std::make_pair(j, std::make_pair(start_times.min(), start_times.max())));
				saved_starts[j] =  std::make_pair(start_times.min(), start_times.max());


				int it = 0;
				// Sample loop to print each element
				for (int i = 0; i < saved_starts.size(); i++)
				{
					Time est = saved_starts[i].first;
					Time lst = saved_starts[i].second;
					std::cout << "Job " << i << ": EST = " << est
							  << ", LST = " << lst << std::endl;
				}

				//DM("This is state: " << &this << std::endl);
				// Nested loop to print each element
				for (size_t i = 0; i < blocked_by_reverse.size(); ++i) {
    			std::cout << "Job " << i << " blocked by: ";
    				for (const Job_index& job : blocked_by_reverse[i]) {
        				std::cout << job << " ";
    				}
    			std::cout << std::endl;
				}
			...
			Time get_max_est_of_lpi(Job_index idx){
				Time max_t = 0;
				for(auto blocking: blocked_by_reverse[idx]){
					if(saved_starts[blocking].first > max_t){max_t = saved_starts[blocking].first;}
				}
				return max_t;
			}
		...
		public:
			...
			// transition: new by scheduling a job 'j' in an existing node 'from'
			Schedule_node(
			...
			{
				stream << "Node id:" << n.lookup_key << " #of states: (" << n.states.size() << ")";
				return stream;
			}
